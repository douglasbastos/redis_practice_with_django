...
def generator_string_by_range(type_string='letters', min=6, max=12, space=False):
    max_length = choice(range(min, max+1))
    # aumenta as chances de vir um vázio colocando 4x
    spa = '    ' if space else ''
    if type_string == 'lowercase':
        return str(''.join(choice(string.ascii_lowercase+spa) for i in range(max_length)))
    elif type_string == 'uppercase':
        return str(''.join(choice(string.ascii_uppercase+spa) for i in range(max_length)))
    else:
        return str(''.join(choice(string.ascii_letters+spa) for i in range(max_length)))
